...
app.get("/api/parental-guide", function(req, res) {
  ...
  axios__default["default"](url).then((response) => {
    let html = response.data;
    let $ = cheerio__default["default"].load(html);
    let posts = [];
    $("section[id^='advisory']:lt(5)", html).not("section[id*='spoilers']").each(function() {
      const title = $(this).find("h4").text();
      const category = $(this).find("span.ipl-status-pill").text();
      const categoryVote = $(this).find("a.advisory-severity-vote__message").text();
      const descList = [];
      $(this).find("li.ipl-zebra-list__item").each(function() {
        descList.push($(this).contents().filter(function() {
...
app.get("/api/watch-guide-v1", function(req, res) {
  ...
  let posts = [];
  axios__default["default"].get(`https://api.themoviedb.org/3/find/${imdbID}?api_key=${tmdbAPIKey}&external_source=imdb_id`).catch(function(error) {
    console.log(error);
  }).then((response) => {
    ...
    axios__default["default"].get(url).catch(function(error) {
      console.log("couldn't find watch options");
    }).then((response) => {
      if (response.data.results.GB) {
        return response.data.results.GB.link;
      } else {
        return response.data.results[Object.keys(response.data.results)[0]].link;
      }
    }).catch(function(error) {
      console.log(error);
    }).then((url2) => {
      axios__default["default"](url2).then((response) => {
        let html = response.data;
        let $ = cheerio__default["default"].load(html);
        $(".ott_provider").filter(function() {
          return $(this).find("h3").text().trim() === "Stream";
        }).find(".ott_filter_best_price").each(function() {
          const watch_platform_title = $(this).find("a").attr("title").split(new RegExp("(?<= on)"))[1].trim();
          const watch_platform_img = tmdb_base_url + $(this).find("img").attr("src");
...
app.get("/api/watch-yts", function(req, res) {
  ...
  let posts = [];
  axios__default["default"].get(MD_url + imdbID).then((response) => response.data.data.movie).then((data) => {
